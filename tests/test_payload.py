...
class Machine:
    def __init__(self,exe,base):
        ...
        self.handlers={}
        self.events=[]
        for rva in [0x1c95050,0x14a5490,0x16442b0,0x16449f0,0x5c5750,0x1628730,0x4a8690,0x1100,
                    0x3b0dbc0,0x7381a0,0x7380c0]:
            self.map(base+rva,1)
        self.u.hook_add(UC_HOOK_CODE,self.hook)
        self.handlers[base+0x1c95050]=lambda a:a[0]+0x30
        self.handlers[base+0x14a5490]=lambda a:a[0]+0x100
        self.handlers[base+0x1100]=lambda a:a[0]+0x120
        self.handlers[base+0x4a8690]=lambda a:self.alloc(a[0]) if a[0] else 0
...
def suite(m):
    ...
    for scope in [0,0x5e0,0x98,0xb8]:
        ...
        m.handlers[m.base+0x16442b0]=receive
    ...
    m.handlers[m.base+0x16442b0]=lambda a:captured.update(items=m.values(a[1]),count=a[4])
    ...
    destroyed=[];seen={}
    m.handlers[m.base+0x5c5750]=lambda a:destroyed.append(bytes(m.u.mem_read(a[0],0x30)))
    def peers_receive(a):
    ...
    m.handlers[m.base+0x16449f0]=peers_receive
    ...
    m.handlers[m.base+0x1628730]=lambda a:seen.update(args=a[:3]) or a[1]
    ...
    for query in ['美 丽','AB',' ab ','!',' ','美丽']:
        ...
        m.handlers[m.base+0x7381a0]=lambda a:seen.append(('construct',a[0])) or a[0]
        m.handlers[m.base+0x7380c0]=lambda a:seen.append(('append',a[0],a[1]))
    ...
    m.handlers[m.base+0x3b0dbc0]=lambda a:seen.append(a[:3]) or a[0]
