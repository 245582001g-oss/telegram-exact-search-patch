...
消息搜索结果右键 → **进入多选屏蔽…**。弹出的批量列表按频道 ID 合并重复结果；点击复选框，或按住右键上下拖过多行。起点未选则一路选中，起点已选则一路取消；拖到列表边缘自动滚动。滚动时勾选保留。也可以全选或取消全选。

点“屏蔽所选频道”会整批保存，再询问是否添加关键词；取消不写规则。只列出进入时已经加载的广播频道，不自动下载额外搜索页。结果数指当前加载的结果行数。频道名和示例只用于本次窗口，频道黑名单仍仅保存固定 ID。

v1.4.1 已修复右键菜单消失时关键词弹窗立刻关闭的问题。批量屏蔽沿用频道规则的逐条撤销功能；不会自动把常见词保存为关键词。

v1.4.2 进一步修复弹窗被主窗口挡住的问题。打开关键词或多选窗口后，先完成或取消当前窗口；重复调用会唤回同一窗口，不会丢失尚未提交的选择和编辑。
