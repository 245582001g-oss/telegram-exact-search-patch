/* Local exact-search patch, bound to Telegram Desktop 7.2.8 SHA-256 in build_patch.py.
 * No new network code, imports, or credential access. Optional content rules
 * persist UTF-16 lengths and SHA-256 hashes; separate channel rules store
 * stable typed channel IDs in Known Documents/Telegram/blacklists.
 * All original
...
static int message_matches(void *item, Text query) {
    ...
    return item && all_keywords(text(FN(0x1c95050,OriginalText)(item)),query);
}
...
static int peer_matches(void *peer, Text query) {
    ...
    if (!peer) return 0;
    if (contains(text(FN(0x14a5490,Name)(peer)),query)) return 1;
...
EXPORT void PatchMessages(void *inner, PtrVector *items, void *inject,
    ...
    int filtering=exact;
    /* Reject blocked channel IDs before retrieving text, matching long spam,
     * or hashing content. The channel database is opened once per page. */
    if (items->end!=items->begin || inject) {
        ...
        if (channels.valid && channels.count) {
            ...
            for (void **p=items->begin; p!=items->end; ++p)
                if (!ch_contains_item(&channels,*p)) *out++=*p;
            items->end=out;
            if (inject && ch_contains_item(&channels,inject)) inject=0;
        }
        bl_close(&channels);
    }
    if (exact) {
    ...
    if (!exact || items->end!=items->begin || inject) {
        ...
        if (blocked.valid && blocked.count) {
            ...
            void **out=items->begin;
            for (void **p=items->begin; p!=items->end; ++p) {
                if (!bl_contains(&blocked,text(FN(0x1c95050,OriginalText)(*p)))) *out++=*p;
            }
            items->end=out;
            if (inject && bl_contains(&blocked,text(FN(0x1c95050,OriginalText)(inject)))) inject=0;
        }
        bl_close(&blocked);
    }
    if (filtering) {
    ...
    FN(0x16442b0,Receive)(inner,items,inject,type,full_count);
    if (filtering) {
...
EXPORT PtrVector *PatchLocal(void *list, PtrVector *result, void *words, void *inner) {
    ...
    if (!active(inner)) return FN(0x1628730,Filtered)(list,result,words);
    Text query=text((u8 *)inner+0x618);
    BlockDb channels;
    ch_open(&channels);
...
EXPORT void *PatchWords(void *out, const void *query, int flags, void *inner) {
    ...
    if (!active(inner)) return FN(0x3b0dbc0,Prepare)(out,query,flags);
    FN(0x7381a0,EmptyList)(out);
    FN(0x7380c0,Append)(out,query);
    return out;
}
#include "menu.h"
