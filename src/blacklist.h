...
static void *bl_alloc(BlockDb *db,uptr n) {
    return n ? BL_IAT(0x5f52940,BlAlloc)(db->heap,8,n) : (void *)0;
}
static void bl_free(BlockDb *db,void *p) {
    if(p) BL_IAT(0x5f524a8,BlFree)(db->heap,0,p);
}
...
static int bl_initialize(BlockDb *db) {
    ...
    db->heap=BL_IAT(0x5f524a0,BlGetHeap)();
    if(!db->heap) { db->error=-1; return 0; }
    db->path=(u16 *)bl_alloc(db,BL_PATH_CAP*2);
    if(!db->path) { db->error=-1; return 0; }
    unsigned int n=BL_IAT(0x5f52118,BlModuleName)(0,db->path,BL_PATH_CAP);
    ...
    void *kernel=BL_IAT(0x5f52148,BlGetModule)((const u16 *)L"kernel32.dll");
    BlFinalPath final_path=kernel ? (BlFinalPath)BL_IAT(0x5f521c0,BlGetProc)(
        kernel,"GetFinalPathNameByHandleW") : (BlFinalPath)0;
    if(!final_path) { db->error=-1; return 0; }
    void *executable=BL_IAT(0x5f52098,BlCreateFile)(db->path,0,7,0,3,0x80,0);
    if(executable==BL_INVALID) { db->error=-1; return 0; }
    n=final_path(executable,db->path,BL_PATH_CAP,0);
    int closed=BL_IAT(0x5f520a8,BlCloseHandle)(executable);
...
static int bl_read_all(void *handle,void *buffer,unsigned int size) {
    ...
    while(size) {
        unsigned int got=0;
        if(!BL_IAT(0x5f521e0,BlReadFile)(handle,p,size,&got,0)
...
static int bl_load(BlockDb *db) {
    db->valid=0;
    void *file=BL_IAT(0x5f52098,BlCreateFile)(db->path,0x80000000u,5,0,3,0x80,0);
    if(file==BL_INVALID) {
        if(BL_IAT(0x5f52078,BlLastError)()==2) { db->valid=1; return 1; }
        db->error=-1; return 0;
    }
    long long size=0;
    u8 header[16];
    int ok=BL_IAT(0x5f52068,BlFileSize)(file,&size)
    ...
    if(!BL_IAT(0x5f520a8,BlCloseHandle)(file)) ok=0;
...
static int bl_crypto(BlockDb *db) {
    ...
    db->crypto_module=BL_IAT(0x5f52398,BlLoadLibrary)((const u16 *)L"bcrypt.dll",0,0x800);
    if(!db->crypto_module) { db->error=-2; return 0; }
    BlGetProc get=BL_IAT(0x5f521c0,BlGetProc);
...
static void bl_close(BlockDb *db) {
    ...
    if(db->crypto_module) BL_IAT(0x5f521c8,BlFreeLibrary)(db->crypto_module);
...
static int bl_write_all(void *file,const u8 *bytes,unsigned int size) {
    while(size) {
        ...
        if(!BL_IAT(0x5f520a0,BlWriteFile)(file,bytes,size,&wrote,0)
...
static int bl_save(BlockDb *db,unsigned int count,const BlockKey *added) {
    ...
    bl_hex(temporary+at,BL_IAT(0x5f52100,BlProcessId)(),8); at+=8;
    temporary[at++]='.';
    bl_hex(temporary+at,BL_IAT(0x5f52220,BlTick)(),16); at+=16;
    temporary[at++]='.'; temporary[at+2]=0;
    for(unsigned int attempt=0;attempt<32;++attempt) {
        bl_hex(temporary+at,attempt,2);
        file=BL_IAT(0x5f52098,BlCreateFile)(temporary,0x40000000u,0,0,1,0x80,0);
        if(file!=BL_INVALID) { created=1; break; }
        unsigned int error=BL_IAT(0x5f52078,BlLastError)();
        if(error!=80 && error!=183) goto done;
    }
    if(file==BL_INVALID) goto done;
    int ok=bl_write_all(file,buffer,bytes)
        && BL_IAT(0x5f52288,BlFlush)(file);
    if(!BL_IAT(0x5f520a8,BlCloseHandle)(file)) ok=0;
    file=BL_INVALID;
    if(!ok) goto done;
    if(!BL_IAT(0x5f52280,BlMove)(temporary,db->path,9)) goto done;
    created=0; result=1;
done:
    if(file!=BL_INVALID) BL_IAT(0x5f520a8,BlCloseHandle)(file);
    if(created) BL_IAT(0x5f52080,BlDelete)(temporary);
...
static int bl_change(int op,const BlockKey *key) {
    ...
    lock=BL_IAT(0x5f52098,BlCreateFile)(lockpath,0xc0000000u,0,0,4,0x80,0);
    ...
    if(lock!=BL_INVALID) BL_IAT(0x5f520a8,BlCloseHandle)(lock);
